...
```
{
  ...
  "scripts": {
    ...
    "deploy": "npm install && npm run build && eb init udagram-api --region us-east-1 && eb list && eb use udagram-api-dev && eb deploy && eb setenv DB_PORT=$DB_PORT JWT_SECRET=$JWT_SECRET POSTGRES_DB=$POSTGRES_DB POSTGRES_HOST=$POSTGRES_HOST POSTGRES_PASSWORD=$POSTGRES_PASSWORD POSTGRES_USERNAME=$POSTGRES_USERNAME URL=$URL",
